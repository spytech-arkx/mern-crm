...
export default function LogInPage() {
  ...
  // sometimes, katrch9 lia chaining syntax instead of a sink and a weight
  const onSubmit = async (data) => {
    await login(data)
      .then((response) => {
        if (response.data) dispatch(userHasAuthenticated(response.data.user));
        if(response.error && response.error.originalStatus !== 401 ) throw 'Login Failed, Problem on server end.'
      })
      .catch((err) => {
        console.error(err);
  ...
  return (
    <>
      <div className="container relative h-screen grid items-center justify-center lg:max-w-none lg:grid-cols-[1fr_1fr_1fr] lg:px-0 bg-gradient-to-r from-slate-900 to-lime-100">
        ...
        <Card className="max-w-sm mx-auto ml-100px">
          <CardHeader>
